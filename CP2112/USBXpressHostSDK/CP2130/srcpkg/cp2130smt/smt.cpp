...
void redirStdinToCfgFile( int argc, const char * argv[])
{
    ...
    {
        throw CUsageErr( "\nERROR: Command line must specify one configuration file");
    }
}
...
// If the m_sN vector contains the passed-in sN, remove sN from m_sN and return true
bool CSerNumSet::findAndErase( const std::vector< BYTE> &sN)
{
    for (std::vector< std::vector< BYTE> >::iterator it = m_SN.begin() ; it != m_SN.end(); ++it)
    {
        if( *it == sN)
        {
            m_matchedSN = sN;    // MCUSW-418: Save matched SN; will be copied to config before verification.
            m_SN.erase( it);
            return true;
        }
    }
...
{
#if (!(ENABLE_INFINITE_LOOPS))
    const DWORD NumDevs = LibSpecificNumDevices(oldVidPid, newVidPid);
    for (int i = 0; i < WAIT_FOR_DEVICES_NUMLOOPS; i++)
    {
        if (NumDevs == expectedCount)
        {
            return;
        }
        std::cerr << "INFO: " << NumDevs << " devices found, waiting for " << expectedCount << ".\n";
        delayMsec(WAIT_FOR_DEVICES_DELAY_MSEC);
    }
    throw CCustErr("ABORT: Expected number of devices not found");
#else // ORIGINAL: Infinite loop
#ifdef _WIN32
