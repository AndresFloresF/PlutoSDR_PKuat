...
int main(int argc, char **argv)
{
	...
	int bsize = 4096;
	rxbuf = iio_device_create_buffer(rx, bsize, false);
	if (!rxbuf)
	{
		perror("Could not create RX buffer");
		shutdown();
	}
	txbuf = iio_device_create_buffer(tx, bsize, false);
	if (!txbuf)
	{
	...
	long ns = rxcfg.fs_hz / bsize;
    if (argc>1) filename = argv[1];
	
	printf("* Iniciando envió de archivo\n");
	...
	/*printf("* Leyendo información enviada\n");
	enum dmode dm;
	...
	for (int c = 0; c < ns; c++)
	{
		...
		if (res != 2 * 2 * bsize)
			printf("  !! warning insufficient number of bytes written, output may be truncated\n");
		if (dm == DUMP_CONTINUOUS)
